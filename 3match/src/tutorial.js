...
export function showTutorial() {
    ...
    if (hint) {
        ...
        const startX = c1 * (cellSize + gap) + gap + (cellSize / 2);
        const startY = r1 * (cellSize + gap) + gap + (cellSize / 2);

        const endX = c2 * (cellSize + gap) + gap + (cellSize / 2);
        const endY = r2 * (cellSize + gap) + gap + (cellSize / 2);
